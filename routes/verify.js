...
const {CreateUser, UpdateUser} = require("../helper/UserHelper")
...
/**
 * verify user with contextId that generated by login method
 */
        ...
        if (data.error === true) {
            ...
            if (data.data.contextIds.includes(token) && data.data.unique) {
                ...
                if (data.data.contextIds.length > 1) {
                    ...
                    let payload = {contextId: token}
                    let jwtToken = jwt.sign(payload, process.env.JWT_SECRET, jwtOptions)
                    res.clearCookie(token)
                    res
                        .status(200)
                        .json({
